...
func newCustodyTimelineItemFromAssetHistoryItem(
	...
	return custodyTimelineItemView{
		...
		TypeLabel:            custodyTimelineTypeLabel(typeLabel),
		TypeClass:            custodyTransactionTypeClass(string(item.Type)),
		TypeTone:             custodyTimelineTypeTone(typeLabel),
		ReceiptURL:           transactionURL,
		RegisteredBy:         militaryDisplayName(item.OperatorRank, item.OperatorAlias),
		RegisteredAt:         custodyTimelineRegisteredAt(formatTimelineDate(item.CreatedAt), formatTimelineTime(item.CreatedAt)),
		PersonnelDisplayName: militaryDisplayName(domain.Rank(item.PersonnelRank), item.PersonnelAlias),
		...
		HasCorrections:       item.EditCount > 0,
